...
def test_ingest_text_creates_document_and_role_instance(client):
    ...
    with db.db_cursor() as cur:
        ...
        role = cur.fetchone()
        cur.execute("SELECT provenance, kind, body FROM jobber.document WHERE id = %s", (role["document_id"],))
        document = cur.fetchone()

    assert role["title"] == "Senior Pricing Actuary"
    assert role["organisation"] == "L&G"
    assert document["provenance"] == "original_capture"
    assert document["kind"] == "job_posting"
    assert "IFRS 17" in document["body"]
...
def test_document_reuse_flag_on_repeated_ingest(client):
    ...
    assert first["id"] != second["id"]  # two distinct role_instance rows can share one document
